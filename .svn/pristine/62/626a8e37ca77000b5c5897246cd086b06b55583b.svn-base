...
function constractor(question) {
    ...
    if (isDeviceMobile && !isiPad) {
        $('.imageDiv').append('<img id="background" src="' + url + 'bg.jpg" />');
        displaySize();
        $('#hintModal').remove();
        ...
        if (document.getElementById("blank") === null || document.getElementById("blank") === 'undefined') {
            $('.imageDiv').append('<img id="blank" src="' + '/Content/images/skilltest/blank.png' + '" usemap="#mapping" />');
        }
        if (document.getElementById("background") === null || document.getElementById("background") === 'undefined') {
            $('.imageDiv').append('<img id="background" src="' + url + 'bg.jpg" />');
        }
        $('.characters').append('<img class="leftPerson" id="' + parameters.leftPersonTag + '" data-tooltip="' + parameters.leftTooltip + '" data-positionx="' + parameters.leftTooltipX + '" data-positiony="' + parameters.leftTooltipY + '" src="' + url + 'left.png" />').
                         append('<img class="rightPerson" id="' + parameters.rightPersonTag + '" data-tooltip="' + parameters.rightTooltip + '" data-positionx="' + parameters.rightTooltipX + '" data-positiony="' + parameters.rightTooltipY + '" src="' + url + 'right.png" />');
        if (question == 1) {
            constractorQuestion_1();
        }
        $('.headersText').append('<h1 id="questionText" class="text">' + parameters.text + '</h1>');//.append('<div id="hintText"><h2>' + hint + '</h2></div>');
        $('#testMenuIcon').hide();
...
function selectPerson(personTag, save) {
    ...
    if (answered >= 9) {
        ...
        $('#continue').text('View results').css({ 'color': '#fff', 'border-color': '#fff' }).attr({ href: hrefURL });
        if (isDeviceMobile && !isiPad) {

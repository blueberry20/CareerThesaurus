...
namespace SkillCowResponsive.Classes.Cloud.TableStorage.AssessmentGroupPinCode
{
    public class AssessmentGroupPinCodeClient : TableServiceClientBase<AssessmentGroupPinCode>
    {
        ...
        public CloudTableQuery<AssessmentGroupPinCode> GetAllByTeacher(string teacher)
        {
            return (from e in TableContext().CreateQuery<AssessmentGroupPinCode>(tableName)
                    where e.Teacher == teacher
                    select e).AsTableServiceQuery<AssessmentGroupPinCode>();
        }
    }
}
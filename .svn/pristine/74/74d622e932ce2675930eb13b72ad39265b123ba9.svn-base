...
{
    public class DirectEmployerClientCampusProgram : TableServiceEntity
    {
        public DirectEmployerClientCampusProgram()
        {
            PartitionKey = "same";
        ...
        public double GetAttributeScore(JObject attributes)
        {
            ...
            if (DimensionAction == null || DimensionAction == "") { score += 0.5; } else if (DimensionAction == attributes["Action"].ToString()) { score += 1; }
            if (DimensionAttitude == null || DimensionAttitude == "") { score += 0.5; } else if (DimensionAttitude == attributes["Attitude"].ToString()) { score += 1; }
            if (DimensionCompensation == null || DimensionCompensation == "") { score += 0.5; } else if (DimensionCompensation == attributes["Compensation"].ToString()) { score += 1; }

            if (DimensionConcentration == null || DimensionConcentration == "") { score += 0.5; } else if (DimensionConcentration == attributes["Concentration"].ToString()) { score += 1; }
            if (DimensionEndurance == null || DimensionEndurance == "") { score += 0.5; } else if (DimensionEndurance == attributes["Endurance"].ToString()) { score += 1; }
            if (DimensionInformation == null || DimensionInformation == "") { score += 0.5; } else if (DimensionInformation == attributes["Information"].ToString()) { score += 1; }

            if (DimensionPatterns == null || DimensionPatterns == "") { score += 0.5; } else if (DimensionPatterns == attributes["Patterns"].ToString()) { score += 1; }
            if (DimensionPresence == null || DimensionPresence == "") { score += 0.5; } else if (DimensionPresence == attributes["Presence"].ToString()) { score += 1; }
            if (DimensionProcessing == null || DimensionProcessing == "") { score += 0.5; } else if (DimensionProcessing == attributes["Processing"].ToString()) { score += 1; }

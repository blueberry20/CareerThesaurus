function createUnifiedForm(unified) {
    ...
    if (false && submitted != null && submitted.indexOf(_school_clientid) >= 0) {

        $('#btnsubmitform').hide();
        $('.formauthcontrols').hide();

        thankscontentdiv.innerHTML = '<div><img src="/content/images/kiosk/importantthings/check.png"/><h1 style="margin: 0px;">Info Requested.</h1><br /><p style="font-size: 12pt;">You will be contacted by a representative shortly.  Please pick up your phone when you see an unfamiliar number.</p></div>';
        formholder.appendChild(thankscontentdiv);
    }
    else {

        clearCookie("campus_key");
        clearCookie("program_key");
        clearCookie("correct_program_key");
        ...
        //headerdiv.innerHTML = '<h1 style="margin: 0px;">Request free info</h1><br />';
        ...
        try {
            ...
            customform.appendChild(createHiddenInput("program_rowkey", _school_programrowkey));
            ...
            if (!customformfields) {
                debugalert('Creating standard fields');
                ...
                switch (f.type) {
                    ...
                    case 'dropdown':
                        try {
                            ...
                            if (!customformfields) {
                                //debugalert('dropdown: ' + f.name);
                                customform.appendChild(createDropdownField(f));
                            }
                            else {
                                if (f.name == 'campus_key') {
                                }
            ...
            if (!customformfields) {
                ...
                createHiddenInput("campus_key", 'Main')
                createHiddenInput("program_key", 'Main')
                createHiddenInput("correct_program_key", 'Main')
            }
            ...
            leadidinput.setAttribute('value', 'yo');

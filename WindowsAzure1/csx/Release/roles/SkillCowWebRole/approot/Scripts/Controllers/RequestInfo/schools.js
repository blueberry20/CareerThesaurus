function findSchools(programid) {
    ...
    $.ajax({
        url: appserverurl + "/Schools/SchoolIndex",
        type: "POST",
        data: {
            z: "",
            p: programid
        ...
        if (response.result == 'ok') {
            ...
            $('#campusschoolcount').html(campuscount);
            $('#onlineschoolcount').html(onlinecount);
            ...
            $('#campus_preloader').hide();
            $('#online_preloader').hide();
            $('#schools').html('Could not find any schools');
        }
    }).fail(function () {
        $('#campus_preloader').hide();
        $('#online_preloader').hide();
...
function getschoolhtml(school) {
    ...
    return '<div class="schoolresult">' +
                ...
                '<input type="button" value="Request Info" onclick="window.open(\'' + school.form.link + '\');"/>' +
                '</div>' +
                '</div>';
}

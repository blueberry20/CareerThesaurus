...
namespace SkillCowResponsive.Classes.Cloud.TableStorage.AssessmentCareerRating
{
    public class AssessmentCareerRatingClient : TableServiceClientBase<AssessmentCareerRating>
    {
        ...
        public CloudTableQuery<AssessmentCareerRating> GetAllByStudent(string student)
        {
        ...
        public CloudTableQuery<AssessmentCareerRating> GetAllBySchoolAndRating(string school, string dolcode, string rating)
        {
            return (from e in TableContext().CreateQuery<AssessmentCareerRating>(tableName)
                    where e.PartitionKey == school && e.DolCode == dolcode && e.Rating == rating
                    select e).AsTableServiceQuery<AssessmentCareerRating>();
        }
        public CloudTableQuery<AssessmentCareerRating> GetAllBySchoolAndGroupAndRating(string school, string dolcode, string schoolGroup, string rating)
        {
            return (from e in TableContext().CreateQuery<AssessmentCareerRating>(tableName)
                    where e.PartitionKey == school && e.Group == schoolGroup && e.DolCode == dolcode && e.Rating == rating
                    select e).AsTableServiceQuery<AssessmentCareerRating>();
        }
    }
}
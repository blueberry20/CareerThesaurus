...
function createValidationForField(f) {
    ...
    if (f.required == true) {
        ...
        switch (f.name) {
            case "zip":
                controlvalidationcode += 'if(!validateZip($("#form_' + f.name + '").html())) { alert("Invalid zip"); $("#form_' + f.name + '").focus(); return false; }\n';
                break;
            case "phone":
                controlvalidationcode += 'if(!validatePhone($("#form_' + f.name + '").html())) { alert("Invalid phone number"); $("#form_' + f.name + '").select(); $("#form_' + f.name + '").focus(); return false; }\n$("#form_' + f.name + '").val(formatPhoneNumber($("#form_' + f.name + '").html()));\n';
                break;
            case "email":
                controlvalidationcode += 'if(!validateEmail($("#form_' + f.name + '").html())) { alert("Invalid email address"); $("#form_' + f.name + '").select(); $("#form_' + f.name + '").focus(); return false; }\n';
                break;
            case "address1":
                controlvalidationcode += 'if(!validateAddress($("#form_' + f.name + '").html())) { alert("Invalid address"); $("#form_' + f.name + '").select(); $("#form_' + f.name + '").focus(); return false; }\n';
                break;
            default: break;
        }
